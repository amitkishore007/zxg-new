...
#if defined(NDEBUG)
# error "Zedexgoldcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7a27884e0b6a15b7c36f7ab97b763c48ea69ab3fdb489e10701a130b494e826e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zedexgoldcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zedexgoldcoin Signed Message:\n";
...
{
    // Zedexgoldcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Zedexgoldcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Zedexgoldcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Zedexgoldcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zedexgoldcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zedexgoldcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7a27884e0b6a15b7c36f7ab97b763c48ea69ab3fdb489e10701a130b494e826e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "07 june, 2018 ZedexGoldCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("142384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528359904;
        ...
        assert(block.hashMerkleRoot == uint256("58ff9dc5b7fc89e7863dec3d370c252e93bbcb8579f17891a6911c2819ce0910"));

        // my code to generate genesis and merkle root
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Zedexgoldcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZedexgoldcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZedexgoldcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZedexgoldcoinMiner(CWallet *pwallet)
{
    printf("ZedexgoldcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zedexgoldcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZedexgoldcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZedexgoldcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZedexgoldcoinMiner, pwallet));
}
